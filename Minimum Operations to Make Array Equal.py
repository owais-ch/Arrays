'''
Minimum Operations to Make Array Equal (GFG)
...
class Solution:
    def minOperations(self, n: int) -> int:
        ...
        if middle%2!=0:
            num_distance=(middle-1)//2
            num_operations+=(num_distance/2)*(2*2+(num_distance-1)*2)

            return int(num_operations)
        else:
            num_distance=(middle-1)//2
            num_operations+=((num_distance/2)*(2*2+(num_distance-1)*2)+n//2)
